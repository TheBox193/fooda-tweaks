...
if (_.isEmpty(localStorage.getItem('email')) || _.isEmpty(localStorage.getItem('USER_ID'))) {
	$.get('https://app.fooda.com/settings/profile').done(function (resp) {
		var $doc = $(resp);
		var email = $doc.find('input#user_email').val();
		var USER_ID = $doc.find('.edit_user').attr('id').replace('edit_user_', '');
...
if (isMenu) {
	...
	if (moneyString) {
		...
		var option3 = $('*[data-selection-option="Above $9"]')[0];
	...
	$items.each(function (index, item) {
		var $item = $(item);
		var itemName = $item.find('.item__name').text();
		var itemId = $item.find('.item__photo__img').attr('src').split('/').pop().split('.')[0];
		var itemScope = item.dataset.vendor_name.trim() + '::' + itemName.trim();
		item.dataset.itemName = itemName;
		item.dataset.itemScope = itemScope;
		item.dataset.itemId = itemId;
		itemIndex.push({ itemId: itemId, itemScope: itemScope });
	});
	...
	$('.filter-bar > ul').append('<div class="toggle__option vote-up">Only my 👍</div>').find('.vote-up').on('click', function () {
		$('.item[data-vote!="up"]').filter(function (i, item) {
			return item.parentNode.nodeName !== 'SPAN';
		}).wrap('<span style="display: none;"/>');
	});

	$('.filter-bar > ul').append('<div class="toggle__option vote-down">Hide my 👎</div>').find('.vote-down').on('click', function () {
		$('.item[data-vote="down"]').filter(function (i, item) {
			return item.parentNode.nodeName !== 'SPAN';
		}).wrap('<span style="display: none;"/>');
	});

	$items.append('<div class="hide-item" title="Hide until page refresh">X</div>').find('.hide-item').on('click', function (ev) {
		$(ev.target.parentNode).remove();
	});

	$items.append('<div class="vote-item"><div class="vote-item-up" data-vote-type="up">👍<div class="vote-item-up-count" /></div><div class="vote-item-down" data-vote-type="down">👎<div class="vote-item-down-count" /></div></div>').find('.vote-item-up, .vote-item-down').on('click', function (ev) {
		var itemId = $(ev.target).closest('.item').data('itemId');
		var previouseVote = getItemVote(itemId);
		var vote = $(ev.target).closest('.vote-item > div').data('voteType');
	...
	$.get(api + 'votes').then(function (data) {
		data.forEach(function (itemVote) {
			var itemId = itemVote.itemId,
			    vote = itemVote.vote,
			    userId = itemVote.userId;


			if (getItem(itemId).length === 0) return;

			if (userId === getUserId()) setItemVote(itemId, vote);
...
var getItemData = function getItemData(itemId, key) {
	return getItem(itemId)[0].dataset[key];
};
var setItemData = function setItemData(itemId, key, value) {
	return getItem(itemId)[0].dataset[key] = value;
};
...
var getItemVote = function getItemVote(itemId) {
	return getItemData(itemId, 'vote');
};
var getVendorName = function getVendorName(itemId) {
	return getItemData(itemId, 'vendor_name');
};
var getItemScope = function getItemScope(itemId) {
	return getItemData(itemId, 'itemScope');
};
function getItemVoteCount(itemId, vote) {
	var count = Number(getItemData(itemId, 'vote' + vote + 's'));
	return _.isFinite(count) ? count : 0;
}

var incrementVoteCount = function incrementVoteCount(itemId, vote) {
	return setItemVoteCount(itemId, vote, getItemVoteCount(itemId, vote) + 1);
};
var decrementVoteCount = function decrementVoteCount(itemId, vote) {
	return setItemVoteCount(itemId, vote, getItemVoteCount(itemId, vote) - 1);
};

var setItemVote = function setItemVote(itemId, vote) {
	return setItemData(itemId, 'vote', vote);
};
...
if (isItem) {
	...
	$('.buy-now-button-for-desktop').mousedown(function () {
		$('#info_instructions').val(function (index, val) {
			var noSilverware = document.getElementById('silverware').checked && !val.toLowerCase().includes('silverware');
			var noNapkin = document.getElementById('napkins').checked && !val.toLowerCase().includes('napkin');
			var result = [val];
...
if (isRestuarants) {
	...
	var extractDate = function extractDate(i, item) {
